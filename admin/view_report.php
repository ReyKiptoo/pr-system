                        ...
                        <div class="card-header bg-transparent px-4">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <button class="btn btn-lg btn-text-primary btn-icon" type="print"><i class="material-icons">print</i></button>
                                </div>
                            </div>
                        </div>
                        <div class="card-body p-4">
                            ...
                            <table id="employee_data" class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    ...
                                    if (isset($_GET['dept'])) {
                                        $deptId = $_GET['dept'];
                                        $query = "SELECT e.employee_id, e.first_name, e.last_name, e.gender, e.phone, e.email , e.nat_id, p.month, p.year, p.salary, p.total_allowances, p.total_deductions, p.net_pay FROM `payroll` AS p INNER JOIN `employee_details` AS e on p.employee_id = e.employee_id INNER JOIN `department_details` AS d on e.department = d.department_id WHERE d.department_id = $deptId";
                                    ...
                                    } else if (isset($_GET['dept']) && isset($_GET['gender'])) {
                                        $deptId = $_GET['dept'];
                                        $gender = $_GET['gender'];
                                        if ($gender == "Male") {
                                            $gender = "M";
                                        ...
                                        $query = "SELECT e.employee_id, e.first_name, e.last_name, e.gender, e.phone, e.email , e.nat_id, p.month, p.year, p.salary, p.total_allowances, p.total_deductions, p.net_pay FROM `payroll` AS p INNER JOIN `employee_details` AS e on p.employee_id = e.employee_id INNER JOIN `department_details` AS d on e.department = d.department_id WHERE d.department_id = $deptId AND e.gender = $gender";
